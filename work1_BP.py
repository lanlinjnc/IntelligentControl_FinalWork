...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        loss_train = loss_fun(y_pred, y_train)  # 预测值和真实值对比
        optimizer.zero_grad()  # 梯度清零
        loss_train.backward()  # 反向传播
        optimizer.step()  # 更新梯度
        loss_steps[epoch] = loss_train.item()  # 保存loss
        running_loss = loss_train.item()
        print(f"第{epoch}次训练，loss={running_loss}".format(epoch, running_loss))
        with torch.no_grad():  # 下面是没有梯度的计算,主要是测试集使用，不需要再计算梯度了
            net.eval()
            y_pred = net(x_test)
            loss_test = loss_fun(y_pred, y_test)
            accuracy_steps[epoch] = loss_test.mean()
            print("测试集的预测准确率", accuracy_steps[epoch])
